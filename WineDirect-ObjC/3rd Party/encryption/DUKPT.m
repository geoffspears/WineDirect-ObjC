...
- (NSData*)clearCounter
{
    ...
    uint64_t intAnd = 0;
    [dataAnd getBytes:&intAnd length:sizeof(uint64_t)];
    
    uint64_t intKSN = 0;
    [dataKSN getBytes:&intKSN length:sizeof(uint64_t)];
...
- (NSData*)setCounter:(uint32_t)counter
{
    ...
    [cleanKSN getBytes:&right length:sizeof(uint32_t)];
...
- (NSData*)xorData:(NSData*)data withData:(NSData*)xorValue
{
    ...
    for (int x = 0; x < data.length / 8; x++) {
        ...
        uint64_t intXor = 0;
        [[xorValue subdataWithRange:NSMakeRange(x * 8, 8)] getBytes:&intXor length:sizeof(uint64_t)];
        
        uint64_t intData = 0;
        [[data subdataWithRange:NSMakeRange(x * 8, 8)] getBytes:&intData length:sizeof(uint64_t)];
...
- (void)keyGen
{
    ...
    [register1 getBytes:&_register1 length:sizeof(uint64_t)];
    
    uint64_t _keyRight = 0;
    [keyRight getBytes:&_keyRight length:sizeof(uint64_t)];
    
    uint64_t _keyLeft = 0;
    [keyLeft getBytes:&_keyLeft length:sizeof(uint64_t)];
    ...
    [register2 getBytes:&_register2 length:sizeof(uint64_t)];
    _register2 ^= _keyRight;
    register2 = [NSData dataWithBytes:&_register2 length:8];
    
    uint64_t _xor1 = 0;
    [[self dataFromString:@"C0C0C0C000000000"] getBytes:&_xor1 length:sizeof(uint64_t)];
    
    uint64_t _xor2 = 0;
    [[self dataFromString:@"C0C0C0C000000000"] getBytes:&_xor2 length:sizeof(uint64_t)];
...
- (void)variantKeys
{
    ...
    uint64_t intXor = 0;
    [dataXor getBytes:&intXor length:sizeof(uint64_t)];
    
    uint64_t intKey = 0;
    [keyLeft getBytes:&intKey length:sizeof(uint64_t)];
    ...
    intKey = 0;
    [keyRight getBytes:&intKey length:sizeof(uint64_t)];
...
- (NSString*)decrypt:(NSString*)data
{
    ...
    for (int x = (int) data.length - 16; x >= 0; x -= 16) {
